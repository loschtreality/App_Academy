...
def movie_names_before_1940
  ...
  movies = Movie.select("id, title, yr").where('yr < 1940')
end
...
def starring(whazzername)
  ...
  names = Actor.select(:name)
  wzn = whazzername.downcase.split('').sort

  names.each do |name|
    split_name =  name.name.downcase.split('').sort
    return name if  wzn.all?{ |x| split_name.include?(x) }
  end
end
